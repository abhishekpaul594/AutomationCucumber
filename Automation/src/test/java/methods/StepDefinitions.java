...
public class StepDefinitions {
    ...
    @When("^I store gmail in notepad$")
    public void storeGmail()
    {

    }

    @And("^I enter radom (FirstName|LastName|Username) in (.+)$")
    public void enterRandomValue(String random,String element) throws Exception {
        predef.waitPageLoad();
        elem=predef.findElement(element);
        ...
        String userName = firstName+lastName+timeStamp;
        if(random.equalsIgnoreCase("firstname"))
        {
            elem.sendKeys(firstName);
        }
        else if(random.equalsIgnoreCase("lastname"))
    ...
    public void switchToTabByName(String tabName) {
        ...
        while (windowIterator.hasNext()) {
            ...
            FluentWait<WebDriver> wait=new FluentWait<WebDriver>(driver).withTimeout(Duration.ofSeconds(20)).pollingEvery(Duration.ofSeconds(2)).ignoring(NoSuchContextException.class);
        }
        if (!tabFound) {
